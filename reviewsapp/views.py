...
def home(request):
  ...
  followed_users = [obj.followed_user for obj in models.UserFollows.objects.filter(user__exact=request.user)]
...
def review_create(request, ticket_id=None):
  ...
  if request.method == 'POST':
    ...
    if all([ticket_form.is_valid(), review_form.is_valid()]) or all([ticket is not None, review_form.is_valid()]):
      if ticket is None:
  ...
  return render(request, 'reviewsapp/review_create.html', context={'review_form': review_form, 'ticket_form': ticket_form, 'ticket': ticket})
...
def follow(request):
  all_users = get_user_model().objects.all()
  followed_users = [obj.followed_user for obj in models.UserFollows.objects.filter(user__exact=request.user)]
  not_followed_users = [user for user in all_users if user.username != request.user.username and user not in followed_users]
  followed_by = [obj.user for obj in models.UserFollows.objects.filter(followed_user__exact=request.user)]
  ...
  if request.method == 'POST':
    ...
    follow.followed_user = get_object_or_404(get_user_model(), username__exact=request.POST['followed_user'])
    follow.save()
...
def follow_delete(request, follow_user_id):
  follow = get_object_or_404(models.UserFollows, user__exact=request.user.id, followed_user__exact=follow_user_id)
  ...
  return render(request, 'reviewsapp/follow_delete.html', {'followed_user': followed_user})